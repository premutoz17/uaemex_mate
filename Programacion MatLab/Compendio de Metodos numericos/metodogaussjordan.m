...
i=1;
j=1;
fprintf('Este programa soluciona sistemas de ecuaciones por el metodo de Gauss-Jordan. \nSi el sistema es 2x2 se mostrará la gráfica.\n\n')
...
%Fin de la grafica
%Inicia el escalonamiento de la matriz haciendo eliminacion abajo y arriba
fprintf('\nAhora se formará la matriz escalonada. Presione ENTER para comenzar.\n'); pause
...
fprintf('Inicia la eliminación hacía abajo:')
    for k=1:c-1
 ...
 %Fin de la eliminacion hacia abajo
 %Eliminacion hacia arriba
 fprintf('Inicia la eliminación hacía arriba:')
    for k=f:-1:2
            for j=k-1:-1:1
                A(j,:)=A(j,:)-A(k,:)*A(j,k);
                j=j-1;
...
%Fin de la eliminacion hacia arriba
